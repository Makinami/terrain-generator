...
struct RenderItem
{
	...
	// Dirty flag indicating the object data has changes and we need to update the constant buffer.
	// Because we have an object cbuffer for each FrameResource, we have to apply the 
	// update to each FrameResource. Thus, when we moidify object data we should set 
...
void TexWavesApp::Update(const GameTimer& gt)
{
	...
	// Cycle through the circulat frame resource array.
	mCurrFrameResourceIndex = (mCurrFrameResourceIndex + 1) % gNumFrameResources;
	mCurrFrameResource = mFrameResources[mCurrFrameResourceIndex].get();

	// Has the GPU finished processing the commands of the current frame resource?
	// If not, wait until the GPU has completed commands up to this fance point.
...
void TexWavesApp::Draw(const GameTimer& gt)
{
	...
	// Bind per-pass constant buffer. We only need to do this onve per-pass.
	...
	// set until the GPU finishes all the commands prior to this Signal().
...
void TexWavesApp::UpdateObjectCBs(const GameTimer & gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		if (e->NumFramesDirty > 0)
		{
			...
			// Next FrameResource need to be updates too.
			e->NumFramesDirty--;
		}
	}
}
...
void TexWavesApp::UpdateMaterialCBs(const GameTimer & gt)
{
	...
	for (auto& e : mMaterials)
	{
		...
		// data changes, it needs to be update for each FrameResource.
...
void TexWavesApp::UpdateWaves(const GameTimer & gt)
{
	...
	if ((mTimer.TotalTime() - t_base) >= 0.25f)
	{
		...
		float r = MathHelper::Rand(0.2f, 0.5f);
	...
	// Set the dynamic VB of the wave renderitem to the current frame.
...
void TexWavesApp::BuildLandGeometry()
{
	...
	//
	// Extract the vertex elements we are interested in and apply the height function to
...
void TexWavesApp::BuildMaterials()
{
	...
	// This is not a good water meterial definition, but we do not have all the rendering
	// tool we need (transparency, environment reflection), so we fake it for now.
