...
DxException::DxException(HRESULT hr, std::wstring_view functionName, std::wstring_view filename, int lineNumber) :
	ErrorCode(hr),
	...
	Microsoft::WRL::ComPtr<ID3D12Resource>& uploadBuffer)
{
	...
	// will copy the CPU memoty into the intermediate upload heap. Then, using ID3D12CommandList::CopySubresourceRegion,
	...
	// the comman list has not been executed yat that performs the actual copy.
	...
	do
	{
		...
		if (errors != nullptr)
		{
			...
			if (auto hr = IDRETRY)
			{
				...
				relative = L"..\\" + relative;
#endif
				ThrowIfFailed(CopyFile((relative + filename).c_str(), filename.c_str(), false));
			}
			else
				break;
		}
