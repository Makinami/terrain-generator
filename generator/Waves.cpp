	...
	if (t >= mTimeStep)
	{
		...
		concurrency::parallel_for(1, mNumRows - 1, [this](int i)
		{
			for (int j = 1; j < mNumCols - 1; ++j)
			{
				...
				XMStoreFloat3(&mNormals[i*mNumCols + j],
					XMVector3Normalize(XMVectorSet(-r + l, 2.0f*mSpatialStep, b - t, 0.0f)));

				XMStoreFloat3(&mTangentX[i*mNumCols + j],
					XMVector3Normalize(XMVectorSet(2.0f*mSpatialStep, r - l, 0.0f, 0.0f)));
			}
		});
	}
}
...
void Waves::Disturb(int i, int j, float magnitude)
{
	...
	// Disturb the ijth vertex height and its neighbours.
